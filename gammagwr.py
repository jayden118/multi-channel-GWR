...
class GammaGWR:
    ...
    def find_bmus_se(self, input_vector, firstDim, alpha, **kwargs) -> Union[Callable[[np.ndarray], Any], Tuple[int, float]]:
        second_best = kwargs.get('s_best', False)
        distances1 = np.zeros(self.num_nodes)
        distances2 = np.zeros(self.num_nodes)
        distances = np.zeros(self.num_nodes)
        for i in range(0, self.num_nodes):
            #print("input: \n {} \n".format(input_vector[:,firstDim:]))
            #print("weights: \n {} \n".format(self.weights[i][:,firstDim:]))
            distances1[i] = self.compute_distance(self.weights[i][:, 0:firstDim], input_vector[:, 0:firstDim]) 
            distances2[i] = self.compute_distance(self.weights[i][:, firstDim:], input_vector[:, firstDim:])
            distances[i] = alpha*distances1[i] + (1-alpha)*distances2[i]
            #print("d1:{} d2:{} = d:{}".format(distances1[i], distances2[i], distances[i]))
        if second_best:
            # Compute best and second-best matching units
            return self.find_bs(distances)
        else:
    ...
    def remove_old_edges2(self) -> None:
        for i in range(0, self.num_nodes):
            neighbours = np.nonzero(self.edges[i])
            v_threshold = np.mean(self.habn) + np.std(self.habn)
            for j in neighbours[0]:
                if self.ages[i, j] >  self.max_age and self.habn[i] > v_threshold:
                    self.edges[i, j] = 0
                    self.edges[j, i] = 0
                    self.ages[i, j] = 0
                    self.ages[j, i] = 0
