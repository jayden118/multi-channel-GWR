...
class EpisodicGWR(GammaGWR):
    ...
    def init_network_mc(self, ds, ds2, e_labels, num_context) -> None:
        ...
        # Start with 2 neurons
        self.num_nodes = 2
        #self.dimension = ds.vectors.shape[1] + ds2.vectors.shape[1]
        self.dimension1 = ds.vectors.shape[1] 
        self.dimension2 = ds2.vectors.shape[1]
        self.dimension = self.dimension1 + self.dimension2
        self.num_context = num_context
        self.depth = self.num_context 
        empty_neuron = np.zeros((self.depth, self.dimension))
        self.weights = [empty_neuron, empty_neuron]
        self.vectorsT = []
        ...
        init_ind = list(range(0, self.num_nodes))
        for i in range(0, len(init_ind)):
            #ds.vectorsT[i] = np.concatenate((ds.vectors, ds2.vectors), axis=1)
            self.vectorsT = np.hstack((ds.vectors[i], ds2.vectors[i]))
            self.weights[i][0] = self.vectorsT
            #print("weights: \n {}".format(self.weights[i][0].shape[0]))
    ...
    def train_egwr(self, ds_vectors, ds_labels, epochs, a_threshold, beta, 
        ...
        self.tau_b = 0.3
        self.tau_n = 0.1
        self.max_nodes = self.samples+1000 # OK for batch, bad for incremental
        self.max_neighbors = 6 # 6
        self.max_age = 600 # 600
        self.new_node = 0.5
        self.a_inc = 1
        self.a_dec = 0.1
        self.mod_rate = 0.01
    ...
    def train_egwr_mc(self, ds_vectors, ds_labels, first_dim, epochs, a_threshold, beta, 
                   l_rates, context, regulated) -> None:
        ...
        for epoch in range(0, self.max_epochs):
            for iteration in range(0, self.samples):
                ...
                # Find the best and second-best matching neurons
                b_index, b_distance, s_index = super().find_bmus_se(self.g_context, self.firstDim, 0.7, s_best = True)
    ...
    def test_mc(self, ds_vectors, ds_labels, firstDim, **kwargs):
        test_accuracy = kwargs.get('test_accuracy', False)
        test_vecs = kwargs.get('ret_vecs', False)
        test_samples = ds_vectors.shape[0]
        self.bmus_index = -np.ones(test_samples)
        ...
        for i in range(0, test_samples):
            input_context[0] = ds_vectors[i]
            # Find the BMU
            b_index, b_distance = super().find_bmus_se(input_context, firstDim, 0.7)
            self.bmus_index[i] = b_index
            self.bmus_weight[i] = self.weights[b_index][0]
            self.bmus_activation[i] = math.exp(-b_distance)
            for l in range(0, len(self.num_labels)):
